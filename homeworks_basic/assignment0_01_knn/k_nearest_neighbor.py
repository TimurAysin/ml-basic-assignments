...
class KNearestNeighbor:
    ...
    def compute_distances_no_loops(self, X):
        """
        Compute the distance between each test point in X and each training pointf
        ...
        dists = np.sqrt(np.sum((np.expand_dims(X, 1) - np.expand_dims(self.X_train, 0)) ** 2, axis=2))
